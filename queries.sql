...
where m.user_id IN (select m.user_id from `moderator` as m, `mainuser` as mu where m.user_id=mu.user_name) order by m.udid;
                    
set of Moderatorsand the groups the are member of.
...
and dt.discussion_title IN (select dt.discussion_title from discussion_thread as dt where dt.group_id=dg.group_id) group by dt.discussion_title;
